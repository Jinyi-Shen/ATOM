...
class InformationGainMC(BaseAcquisitionFunction):
    ...
    def sample_representer_points(self):
        ...
        mc = sampler.run_mcmc(start_points, 100)
        self.zb = mc.coords
        self.lmb = mc.log_prob
    ...
    def sampling_acquisition_wrapper(self, x):
        if np.any(x < self.X_lower) or np.any(x > self.X_upper):
            return -np.inf
        if VU_PRINT >= 0:
            print("in ig_mc_freeze, sampling_acquisition_wrapper, : x = ", x, " self.sampling_acquisition = ",
                  self.sampling_acquisition, " np.array[x] = ", np.array([x]))
    ...
    def update(self, model, calc_repr=False):  # Different
        ...
        self.pmin = joint_pmin(self.Mb, self.Vb, self.Nf)
    ...
    def change_pmin_by_innovation(self, x):
        ...
        return joint_pmin(Mb_new, Vb_new, self.Nf)
